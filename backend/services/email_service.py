...
_INTERVIEW_SCHEDULED_HTML = """\
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Interview Scheduled &mdash; JOBEZEE</title>
  <style>
    body {{ margin:0; padding:0; background:#f1f5f9; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif; }}
    .wrapper {{ max-width:560px; margin:40px auto; background:#ffffff; border-radius:16px; overflow:hidden; box-shadow:0 4px 24px rgba(0,0,0,0.08); }}
    .header {{ background:linear-gradient(135deg,#0f172a,#1e293b); padding:36px 40px 28px; }}
    .logo {{ font-size:15px; font-weight:800; letter-spacing:2px; color:#ffffff; text-transform:uppercase; }}
    .logo em {{ color:#06b6d4; font-style:normal; }}
    .header-title {{ margin:18px 0 4px; font-size:20px; font-weight:700; color:#ffffff; }}
    .header-sub {{ margin:0; font-size:13px; color:#64748b; }}
    .body {{ padding:36px 40px; }}
    .card {{ background:#f8fafc; border:1px solid #e2e8f0; border-left:4px solid #06b6d4; border-radius:10px; padding:20px 24px; margin-bottom:24px; }}
    .card-row {{ display:flex; gap:8px; margin-bottom:6px; font-size:14px; }}
    .card-label {{ color:#94a3b8; width:90px; flex-shrink:0; font-weight:600; font-size:12px; text-transform:uppercase; letter-spacing:0.5px; padding-top:1px; }}
    .card-value {{ color:#0f172a; font-weight:500; }}
    .pill {{ display:inline-block; background:#e0f2fe; color:#0369a1; border-radius:20px; padding:2px 10px; font-size:12px; font-weight:600; }}
    .btn-wrap {{ text-align:center; margin:28px 0; }}
    .btn {{ display:inline-block; background:linear-gradient(135deg,#06b6d4,#0ea5e9); color:#ffffff !important; text-decoration:none; font-weight:700; font-size:15px; padding:14px 36px; border-radius:10px; }}
    p {{ margin:0 0 16px; font-size:14px; line-height:1.7; color:#475569; }}
    .divider {{ border:none; border-top:1px solid #e2e8f0; margin:24px 0; }}
    .footer {{ padding:18px 40px 24px; font-size:12px; color:#94a3b8; background:#f8fafc; }}
    .footer a {{ color:#06b6d4; text-decoration:none; }}
  </style>
</head>
<body>
  <div class="wrapper">
    <div class="header">
      <div class="logo">JOB<em>EZEE</em></div>
      <p class="header-title">Interview scheduled</p>
      <p class="header-sub">Your mock interview is locked in &mdash; don&rsquo;t miss it</p>
    </div>
    <div class="body">
      <p>Hi {name},</p>
      <p>Your AI mock interview has been scheduled. Show up ready &mdash; the questions are already generated and waiting.</p>
      <div class="card">
        <div class="card-row"><span class="card-label">Role</span><span class="card-value">{job_title}</span></div>
        <div class="card-row"><span class="card-label">Company</span><span class="card-value">{company}</span></div>
        <div class="card-row"><span class="card-label">Round</span><span class="card-value"><span class="pill">{round_label}</span></span></div>
        <div class="card-row"><span class="card-label">Duration</span><span class="card-value">{duration} minutes</span></div>
        <div class="card-row"><span class="card-label">Scheduled</span><span class="card-value">{scheduled_at}</span></div>
      </div>
      <div class="btn-wrap">
        <a href="{app_url}" class="btn">Open Interview Dashboard</a>
      </div>
      <hr class="divider" />
      <p style="font-size:13px;color:#94a3b8;">Can&rsquo;t make it? You can cancel or reschedule from the Interview tab in your dashboard.</p>
    </div>
    <div class="footer">
      &copy; 2026 JOBEZEE &middot; <a href="https://www.jobezee.org/privacy">Privacy</a> &middot; <a href="https://www.jobezee.org/terms">Terms</a>
    </div>
...
async def send_interview_scheduled_email(
    ...
    duration:    int,
    scheduled_at: str,
    app_url:     str = "https://www.jobezee.org/app/interview",
) -> None:
    """Send a confirmation email when the user schedules a mock interview."""
    html = _INTERVIEW_SCHEDULED_HTML.format(
        name         = name or "there",
    ...
    text = (
        f"Hi {name or 'there'},\n\n"
        f"Your mock interview is scheduled:\n\n"
        f"  Role:      {job_title}\n"
        f"  Company:   {company}\n"
        f"  Round:     {round_label}\n"
        f"  Duration:  {duration} minutes\n"
        f"  Scheduled: {scheduled_at}\n\n"
        f"Open your dashboard to start when ready: {app_url}\n\n"
        f"— The JOBEZEE Team"
    )
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(
        ...
        _send_smtp,
        to_email,
        f"Interview scheduled — {job_title} ({round_label})",
        html,
        text,
    )
