...
import { SophiaSession } from './SophiaSession'
...
const InterviewPage = () => {
  ...
  const handleFinish = async (result: AvatarResult) => {
    setSaving(true)
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Interviewer</span>
        <div className="flex items-center gap-1.5 rounded-full border border-violet-200 bg-violet-50 px-3 py-1.5">
          <div className="w-5 h-5 rounded-full bg-gradient-to-br from-violet-500 to-cyan-400 flex items-center justify-center shrink-0">
            <svg className="w-2.5 h-2.5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
              <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>
            </svg>
          </div>
          <span className="text-xs font-semibold text-violet-700">Sophia</span>
          <span className="text-[10px] text-violet-400 ml-0.5">AI Interviewer</span>
        </div>
      </div>
