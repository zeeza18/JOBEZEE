...
const ResumeMakerPage = () => {
  const { loaded, saving, title, content, settings, load, setTitle } = useResumeMaker()
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div className="lg:sticky lg:top-4 lg:self-start space-y-3">
            <div className="flex items-center justify-end gap-2">
              <a
                href={resumeBuilderApi.exportPdfUrl()} target="_blank" rel="noreferrer"
              ...
              <a
                href={resumeBuilderApi.exportDocxUrl()} target="_blank" rel="noreferrer"
                className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50"
              >
                <FileText className="h-3.5 w-3.5" /> DOCX
              </a>
            </div>
            <div className="max-h-[calc(100vh-160px)] overflow-y-auto rounded-2xl bg-slate-100 p-4">
              <ResumePreview content={content} settings={settings} />
            </div>
          </div>
        </div>
      )}
