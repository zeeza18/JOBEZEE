...
- File upload sequence (CRITICAL — do NOT put any other tool call between click and upload):
  Step 1: browser_click the upload button/area to open the file chooser.
  Step 2: IMMEDIATELY call browser_file_upload with the PDF path. No snapshot, no other calls in between.
  If browser_file_upload returns "no modal state": the visible button is NOT a native file input. Fix: use browser_evaluate with script `document.querySelector('input[type="file"]').click()` to trigger the hidden native input, then immediately call browser_file_upload.
  If multiple file inputs exist, target the correct one: `document.querySelectorAll('input[type="file"]')[0].click()` (try index 1 if index 0 doesn't work).
