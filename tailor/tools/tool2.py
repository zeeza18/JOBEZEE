...
class ResumeTailor:
    ...
    def __init__(self, api_key: str | None = None, fallback_key: str | None = None):
        ...
        self.model = "claude-sonnet-4-6"  # Sonnet via OpusMax: quality tailoring
    ...
    def tailor_resume(
        ...
        # --- Keywords snapshot ---
        job_title = keywords.get('job_title', '').strip()
        keywords_snapshot = (
            f"JD_JOB_TITLE (use this EXACTLY as the resume header title): {job_title or 'extract from JD'}\n"
    ...
    def _build_user_message(
        ...
        # Add Python-verified ground-truth analysis (injected before feedback)
        if pre_analysis:
            analysis_lines = ["\n═══ PYTHON-VERIFIED ANALYSIS (GROUND TRUTH — HIGHEST PRIORITY) ═══"]
            missing = pre_analysis.get('confirmed_missing_from_bullets', [])
            if missing:
                analysis_lines.append(
                    f"MUST INSERT — these keywords are NOT in any bullet yet: {', '.join(missing)}"
                )
            orphaned = pre_analysis.get('confirmed_orphaned_skills', [])
            if orphaned:
                analysis_lines.append(
                    f"ORPHANED SKILLS — in Skills section but no bullet backup: {', '.join(orphaned)}"
                )
            weak = pre_analysis.get('weak_bullets', [])
            if weak:
                analysis_lines.append(
                    "WEAK VERB BULLETS — add metrics or strengthen: " + " | ".join(weak[:3])
                )
            ...
            with open(filepath, 'w', encoding='utf-8') as f:
                ...
                for change in tailored_data.get('change_log', []):
                    if isinstance(change, dict):
                        # Handle both 'description' (eval prompt) and 'change' (round1 prompt) keys
                        desc = change.get('description', '') or change.get('change', '') or ''
                        f.write(f"• [{change.get('type', 'change')}] {desc}\n")
                    else:
                        f.write(f"• {change}\n")
